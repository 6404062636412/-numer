...
const Trapezoidal = () => {
    ...
    const calculateTrapezoidal = (equation, a, b, n) => {
        const h = (b - a) / n;
        let sum = 0;
    ...
    const calculateIntegration = () => {
        ...
        if (!isNaN(aNum) && !isNaN(bNum) && nNum > 0) {
            calculateTrapezoidal(equation, aNum, bNum, nNum);
        } else {
    ...
    const API = async () => {
        try {
            const response = await axios.get("http://localhost:5555/api/getData/Trapezoidal"); // แก้ endpoint
    ...
    return (
        <Container>
            ...
            <div>
                {result !== null && (
                    <p>ผลการคำนวณโดยใช้กฎ Trapezoidal = {result.toFixed(6)}</p>
                )}
            </div>
        </Container>
    );
