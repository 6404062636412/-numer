...
const GaussSeidel = () => {
  ...
  const calculateGaussSeidel = () => {
    ...
    for (let it = 0; it < iterations; it++) {
      ...
      iterationResults.push({ iteration: it + 1, values: [...X] });

      if (X.every((val, idx) => Math.abs(val - X_prev[idx]) < 1e-6)) {
        break;
      }
    }
  ...
  const API = async () => {
    try {
      const response = await axios.get('http://localhost:5555/api/getData/GaussSeidel');
      ...
      setMatrixA(matrixA);
      setMatrixB(matrixB);
      setInitialX(Array.isArray(initialX) ? initialX : Array(rows).fill(0));
    } catch (error) {
  ...
  return (
    <div>
      ...
      <button onClick={calculateGaussSeidel}>Calculate</button>
      {result.length > 0 && (
        <div>
